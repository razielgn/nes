...
impl<'rom> Memory<'rom> {
        ...
        where A: Into<u16>
    {
        ...
        match addr {
            ...
            0x2000...0x3FFF => panic!("read from PPU register ({:04X})", addr),
            ...
            0x8000...0xBFFF => self.rom.prg[self.bank1 * 0x4000 + (addr - 0x8000)],
            0xC000...0xFFFF => self.rom.prg[self.bank2 * 0x4000 + (addr - 0xC000)],
            _ => unreachable!(),
        }
    }
              ...
              V: Into<u8>
    {
        ...
        match addr {
            ...
            0x2000...0x3FFF => panic!("write in PPU register ({:04X})", addr),
